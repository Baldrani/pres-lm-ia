...
function drawGround() {
  ...
  ctx.fillRect(
    ...
    30
  ); // Extend the ground to fit the last slide
}
...
function drawBillboards() {
  slides.forEach((slide, index) => {
    ...
    ctx.fillText(
      slide[0],
      textX - scrollOffset - ctx.measureText(slide[0]).width / 2,
      headerY
    );
    ...
    slide.slice(1).forEach((line, lineIndex) => {
      if (lineIndex < visibleLines[index] - 1) {
        ...
        if (line.match(/\.(jpg|png|webp)$/)) {
          ...
          ctx.fillText(
            line,
            textX - scrollOffset - ctx.measureText(line).width / 2,
            lineY
          );
        }
      }
...
function updatePlayerPosition() {
  ...
  if (player.x < SCREEN_WIDTH / 3 && player.dx < 0 && scrollOffset <= 0) {
